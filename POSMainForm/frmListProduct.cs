...
namespace POSMainForm
{
    public partial class frmListProduct : Form
    {
        ...
        public void LoadProducts(string strSearch)
        {
            try
            {
                SQLConn.sqL = "SELECT ProductNo, ProductCode, P.Description, Barcode, C.CategoryName, UnitPrice, StocksOnHand, ReorderLevel, UnitCost FROM Product as P LEFT JOIN Category C ON P.CategoryNo = C.CategoryNo WHERE P.Description LIKE '%" + strSearch + "%' ORDER BY Description";
                SQLConn.ConnDB();
                ...
                while (SQLConn.dr.Read() == true)
                {
                    ...
                    x.SubItems.Add(Strings.Format(SQLConn.dr["UnitCost"], "#,##0.00"));
